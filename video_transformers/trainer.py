...
from video_transformers.tasks.multi_label_classification import MultiLabelClassificationTaskMixin
...
class BaseTrainer:
    ...
    def _set_loss_function(self, loss_function):
        if self.task == "multi_label_classification" and loss_function is None:
            self.loss_function = torch.nn.BCEWithLogitsLoss()
        elif self.task == "single_label_classification" and loss_function is None:
...
class MultiLabelClassificationTrainer(MultiLabelClassificationTaskMixin, BaseTrainer):
    pass

def trainer_factory(task: str, **kwargs):
    if task == "single_label_classification":
        return SingleLabelClassificationTrainer(task=task, **kwargs)
    elif task == "multi_label_classification":
        return MultiLabelClassificationTrainer(task=task, **kwargs)
    else:

...
class MultiLabelLinearHead(nn.Module):
    """
     (BxF)
       ↓
    Dropout (optional)
    ...
    """
    def __init__(self, hidden_size: int, num_classes: int, dropout_p: float = 0.0):
        super(MultiLabelLinearHead, self).__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: BxF - B is the batch size, F is the feature size
        if self.dropout is not None:
            x = self.dropout(x)
        x = self.linear(x)
        # No activation function is applied here, as BCEWithLogitsLoss will handle it
        return x
